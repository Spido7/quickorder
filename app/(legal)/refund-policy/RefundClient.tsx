    ...
    async function fetchCafe() {
      try {
        ...
        const { data } = await supabase
          ...
          .single();
        if (data) {
          if (data.business_name) setCafeName(data.business_name);
          if (data.phone_number) setCafePhone(data.phone_number);
        }
      } catch (e) {
      ...
      <div className="border-3 border-black bg-white p-6 sm:p-8 shadow-[6px_6px_0px_0px_#000] space-y-6">
        ...
        <section className="space-y-2 border-t-2 border-black/10 pt-4">
          ...
          <ul className="list-disc pl-5 text-sm text-gray-600 space-y-1 font-sans">
            ...
            <li>Please bring the item directly to the counter of <strong className="text-black">{cafeName}</strong> immediately along with your order screen or printed receipt.</li>
